...
class TestInterestRate:
    ...
    def test_create_interest_rate(self):
        ...
        rate = InterestRate(0.05, CompoundingConvention.MONTHLY)
    ...
    def test_discount_factor_monthly(self):
        ...
        df_1y = rate.discount_factor(1.0)

        # Expected: 1 / (1.005)^12 ≈ 0.9419
        assert abs(df_1y - 0.9419) < 0.001
    ...
    def test_compound_factor_monthly(self):
        ...
        cf_1y = rate.compound_factor(1.0)

        # Expected: (1.005)^12 ≈ 1.0617
        assert abs(cf_1y - 1.0617) < 0.001
    ...
    def test_discount_and_compound_are_inverses(self):
        ...
        df = rate.discount_factor(1.0)
        cf = rate.compound_factor(1.0)
    ...
    def test_convert_to_different_compounding(self):
        ...
        cf_monthly = rate_monthly.compound_factor(1.0)
        cf_annual = rate_annual.compound_factor(1.0)

        assert abs(cf_monthly - cf_annual) < 0.0001
...
class TestSpread:
    ...
    def test_create_spread(self):
        ...
        spread = Spread(250.0)
        assert spread.basis_points == 250.0
    ...
    def test_to_decimal(self):
        ...
        spread = Spread.from_bps(250)
        assert spread.to_decimal() == 0.025

    def test_to_percent(self):
        """Test converting spread to percentage."""
        spread = Spread.from_bps(250)
        assert spread.to_percent() == 2.5
